...
3:51PM, done. X E.
5/6/2025, 4:32PM, done. X E.
7/15/2025, 6:56PM, done better. X E.
7/23/2025, 11:10AM, start revamp. X E.
11:14AM, done revamp. X E.
=end
=begin
...
class RinomMathXE
  ...
  def self.shapeDistance(viewPoint,points,points2d,indices,xyz,indexer=Array.new(2,0),numbers=Array.new(2,0.0))
    if (indices.length==0)
      #X E
      return [-2.0,-1.0];
      #X E
    ...
    while (indexer[0] != -1) do
      ...
      numbers[0]+=points[indices[indexer[0]]][xyz[2]]-viewPoint[xyz[2]];
      numbers[1]+=Math.sqrt((points[indices[indexer[0]]][xyz[0]]-viewPoint[xyz[0]])*(points[indices[indexer[0]]][xyz[0]]-viewPoint[xyz[0]])+
                            (points[indices[indexer[0]]][xyz[1]]-viewPoint[xyz[1]])*(points[indices[indexer[0]]][xyz[1]]-viewPoint[xyz[1]]));
      indexer[0]-=1;
    end
    #X E
...
class RinomDrawXE
  ...
  def setDrawOrder()
    ...
    @dists[@indices[0]]=RinomMathXE.shapeDistance(@viewPoint,@points,@points2d,@shapes[@indices[0]],@xyz,@indices1,@numbers);
    @dists[@indices[0]-1]=RinomMathXE.shapeDistance(@viewPoint,@points,@points2d,@shapes[@indices[0]-1],@xyz,@indices1,@numbers);
    #initial compare.
    if (@dists[@indices[0]][0]<@dists[@indices[0]-1][0])
      @drawOrder[0]=@indices[0];
      @drawOrder[1]=@indices[0]-1;
    elsif (@dists[@indices[0]][0]>@dists[@indices[0]-1][0])
      @drawOrder[0]=@indices[0]-1;
      @drawOrder[1]=@indices[0];
    elsif (@dists[@indices[0]][1]<@dists[@indices[0]-1][1])
      @drawOrder[0]=@indices[0];
      @drawOrder[1]=@indices[0]-1;
    else
      @drawOrder[0]=@indices[0]-1;
    ...
    @indices[0]-=2;
    #7/11/2025, 6:11PM, start revamp. 11:46PM, break. X E.
    #7/15/2025, 6:22PM, start again. 6:53PM, done. X E.
    while (@indices[0] != -1) do
      @dists[@indices[0]]=RinomMathXE.shapeDistance(@viewPoint,@points,@points2d,@shapes[@indices[0]],@xyz,@indices1,@numbers);
      #Set up bounds.
      @indices[1]=0;
      @indices[2]=@indices[3]-1;
      if (@dists[@indices[0]][0] < -0.0)
        @indices[1]=@indices[3];
      else
        while (@indices[1] < @indices[2]) do
          #Compare.
          @indices1[1]=(@indices[1]+@indices[2]+1)/2;
          @indices1[0]=@drawOrder[@indices1[1]];
          if (@dists[@indices[0]][0]<@dists[@indices1[0]][0])
            @indices[2]=@indices1[1]-1;
          elsif (@dists[@indices[0]][0]>@dists[@indices1[0]][0])
            @indices[1]=@indices1[1]+1;
          elsif (@dists[@indices[0]][1]<@dists[@indices1[0]][1])
            @indices[2]=@indices1[1]-1;
          elsif (@dists[@indices[0]][1]>@dists[@indices1[0]][1])
            @indices[1]=@indices1[1]+1;
          else
        ...
        if (@indices[1] == @indices[2])
          ...
          if (@dists[@indices[0]][0]>@dists[@indices1[0]][0]||(@dists[@indices[0]][0]==@dists[@indices1[0]][0]&&@dists[@indices[0]][1]>@dists[@indices1[0]][1]))
            @indices[1]+=1;
          end
        end
      end
